...
COMMANDS HOSPEDAJES
*********************
From iocode-auth-server-db Terminal (C:\My_Folder\jsknoGithubProjects\security\starting_guide\iocode-auth-server-db)
ls (ci/db)
*********************
...
docker image build -t auth-server-db:1.0 -f ci/Dockerfile-db .
docker image ls
docker container run -p 5432:5432 auth-server-db:1.0